            ...
            <body className={`${inter.className} antialiased selection:bg-purple-500/30`}>
                <div className="flex h-screen overflow-hidden bg-[#0a0a0b]">
                    ...
                    <main className="flex-1 relative overflow-y-auto overflow-x-hidden">
                            ...
                            <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-white/10 bg-[#0a0a0b]/80 px-8 backdrop-blur-md">
                                ...
                                <div className="flex items-center gap-4">
                                    <div className="flex flex-col items-end">
                                        <span className="text-sm font-medium">{session.email.split('@')[0]}</span>
                        ...
                        <div className={`${session ? 'p-8' : ''} animate-fade-in text-white h-full`}>
                            {children}
                        </div>
                    </main>
                </div>
        ...
        <aside className="w-64 border-r border-white/10 bg-[#0a0a0b] flex flex-col hidden lg:flex">
            ...
            <div className="p-4 border-t border-white/10 space-y-4">
                <div className="glass-card rounded-xl p-4 text-[10px]">
                    <p className="text-white/40 mb-2 uppercase tracking-widest font-bold">{isSuperAdmin ? 'Full Access' : planName}</p>
                    <div className="w-full bg-white/10 h-1 rounded-full overflow-hidden">
                        <div className="bg-purple-600 h-full w-full shadow-sm shadow-purple-600/50" />
                    </div>
                    <p className="mt-2 text-white/60 font-medium truncate">{isSuperAdmin ? 'AppVendix Platform' : companyName}</p>
                </div>
